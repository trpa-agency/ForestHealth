...
## 2026-09-24 (handoff to Andy McClary; sample design template)
- `notebooks/00_sample_design_template.ipynb` from `scripts/build_sample_design_template.py`: the one entry point. Sections 0 to 7 run sources and SDE discovery, the raster frame with the report Table 1 check and the Wilderness question, strata from the F: threshold rasters or the LiDAR metrics (`strata.source`), the split allocation with `sample_size.detectable_margin`, the spsurvey selection through `scripts/grts_split_draw.R` with the Python GRTS as fallback and a `run.freeze` seed guard, evaluation, and export with owl and fine hex keys. Runs end to end in `run.synthetic: true` mode without arcpy, F:, or R.
- `src/layers.py`: `read_layer` and `read_raster` accept `.sde` and `.gdb` paths through arcpy (`arcpy.da.SearchCursor` and `RasterToNumPyArray`); `resolve_source` picks a form from a mapping entry (`rrk`, `lidar`, `sde`, `file`); `read_raster_attribute_table`. File and REST behaviour unchanged; arcpy is imported only when needed.
- `src/design_template.py` (new): `SourceReader`, the synthetic Basin as rasters and layers, SDE discovery and source status, frame helpers, the rrk or lidar proxy shim for `strata.classify_cells`, split allocation on top of `strata.allocate`, the R call and Python fallback, level labels and one installation order for both halves, VP9 and VP10 class helpers, hex keys.
- `config.yaml`: `sde:` block; `sources:` entries carry `rrk:` and `lidar:` (rasters) or `sde:` and `file:` (vectors) forms with `where` and `field` options and `# CONFIRM` placeholders; `strata.source`; `run.rscript`; `frame.exclude_wilderness`; `frame.population_tolerance_pct`; `split_sample.balance.elev_bands_m`. Every existing key and value kept.
- `src/strata.py`: `grts_draw` now runs the systematic selection along the hierarchical line and applies the reverse hierarchical order to the selected sites, as spsurvey does. The earlier order gave samples no more balanced than simple random sampling (quadrant chi-square per df 1.1 against 0.94 for SRS; 0.27 after the fix, 0.47 for a 20 site prefix).
- `README.md` rewritten as Andy's entry point; `docs/HANDOFF-ANDY-2026-09-24.md` added.
