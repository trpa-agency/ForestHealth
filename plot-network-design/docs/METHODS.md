...
1. **Frame** (`01_frame.ipynb`): map CWHR to forest type; remove urban and wilderness; lay the sampling units, `frame.unit_px` by `frame.unit_px` blocks of 30 m LiDAR pixels anchored at the raster origin (3 by 3, 90 m, 0.81 ha), keeping a block when its centre pixel is in the population and at least `frame.min_unit_fraction` of its pixels are; reduce the LiDAR metrics to block means over population pixels and sample the DEM and road distance at the block centre; flag state, owner, fire, treatment; remove edge buffers and slopes above the cutoff; assign access class; write `frame_accounting.csv`. `acres` is the population area a unit represents, `unit_acres` the block area.
...
- The Python GRTS is for iteration; the frozen draw is `spsurvey::grts()` for parity with TEON's backbone. Both enforce `draw.min_distance_m` (120 m, two macroplot radii) between every pair of selected sites, legacy included, so plot footprints never overlap; the split draw also removes units within that distance of Half A sites before drawing Half B.
- The plot centre is the centre pixel of the selected block and is never moved within the unit.
