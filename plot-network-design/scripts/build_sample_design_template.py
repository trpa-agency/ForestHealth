"""Generate notebooks/00_sample_design_template.ipynb. Run: python scripts/build_sample_design_template.py

Edit this builder, regenerate, never hand-edit the .ipynb (repo convention). Execute with:
    python -m jupyter nbconvert --to notebook --execute --inplace notebooks/00_sample_design_template.ipynb
With run.synthetic: true it runs anywhere (no arcpy, no F:, no R). With run.synthetic: false it
needs the arcgispro-py3 interpreter, F: mounted, and Rscript on run.rscript.
...
HEADER = '''import sys, os, time, json, shutil, subprocess, warnings
from pathlib import Path
...
nb("00_sample_design_template", [

# ------------------------------------------------------------------ title
("md", """# 00 Sample design template
...
("code", '''status = dt.source_status(cfg)
...
**Wilderness and urban.** Both exclusions now come from dedicated layers on maps.trpa.org (Mason, Sept 24, 2026): `sources.wilderness` is the congressionally designated Forest Service wilderness (Boundaries layer 13) and `sources.urban_boundary` is the parcel-aligned Urban Area (Boundaries layer 14). The threshold notebook used the RegionalLandUse plan classes instead, keeping its Wilderness class, and the crosswalk read from `veg_type_nonurban` totals 115,600 acres that way. The population check below prints acres by type against 115,396 with the dedicated layers applied; if it lands more than `frame.population_tolerance_pct` off, the report used the plan classes and the `sde` forms of both sources reproduce it.
...
("code", '''veg, TR, RCRS = reader.raster("veg_type")
...
("code", '''# terrain on the strata grid: the 0.7 m SDE bare earth is projected and snapped to the veg raster on a real run
...
("code", '''# sampling units: k x k blocks anchored at the raster origin, plot on the centre pixel
...
("code", '''# unit level exclusions with accounting, then the access class
n0 = len(units)
...
("code", '''# balance covariates: TPI class, basin side, elevation band
...
("code", '''LEVELS = list(cfg["allocation"]["levels"])
...
("code", '''res = None if SYN else dt.run_rscript(cfg, Path("scripts/grts_split_draw.R"), [cfg["draw"]["seed"], cfg["draw"]["oversample_factor"], cfg["draw"]["min_distance_m"]], Path.cwd(), log)
...
("code", '''# nested levels and one installation order for both halves; blind remeasurement flag
...
("code", '''sel = pd.read_parquet(P / "selected.parquet")
...
("code", '''# 4. VP9 and VP10 class coverage against the RRK rasters
...
("code", '''owl_key, fine_key, hex_src = dt.hex_keys(sel["x"], sel["y"], cfg, O / "tessellation_lattice_parameters.csv", log)
